...
```java
ctx.kernel().order("service:register",
...
```java
OrderResult r = ctx.kernel().order("service:get",
...
```java
OrderResult r = ctx.kernel().order("service:get:meta",
    ctx.getPluginId(), "getEnergyGrid");
...
```java
OrderResult r = ctx.kernel().order("service:list");
